...
//#ifdef DEBUG
	...
	//cout << "Packets (Streams or Number of CUDA blocks in x-dimension): " << burst.().size() << endl;
...
//#endif
	...
	cout << tmp_filter_total_size << endl;
	//GPUMemInfo();
	...
	// // Memory allocation for determining frequency of number of states active per group
	// unsigned long long int h_active_bits[BITS] = {0};
	// unsigned long long int *d_active_bits;
	// //h_active_bits = (unsigned long long int *)malloc(BITS*sizeof(unsigned long long int));
	// cudaMalloc((void **) &d_active_bits, BITS*sizeof(unsigned long long int));
	// cudaMemcpy(d_active_bits, h_active_bits, BITS*sizeof(unsigned long long int), cudaMemcpyHostToDevice);


	// // Memory allocation for determining number of groups active for each cycle
	// // int max = 0;
	// // for ( int i = 0; i < cfg.get_alphabet_size(); i++) {
	// // 	if (tg[0]->get_filter_symbol_counts()[i] > max) {
	// // 		max = tg[0]->get_filter_symbol_counts()[i];
	// // 	}
	// // }
	// int active_groups_size = cfg.get_state_vector(0).get_size()/sizeof(ST_BLOCK) + 1;
	// unsigned long long int h_num_active_groups[active_groups_size] = {0};
	// unsigned long long int *d_num_active_groups;

	// cudaMalloc((void **) &d_num_active_groups, active_groups_size*sizeof(unsigned long long int));
	// cudaMemcpy(d_num_active_groups, h_num_active_groups, active_groups_size*sizeof(unsigned long long int), cudaMemcpyHostToDevice);

	// // Memory allocation for determining frequency of number of transitions per thread
	// unsigned long long int bins = 10000;
	// unsigned long long int h_trans_per_thread[bins] = {0};

	// unsigned long long int * d_trans_per_thread;

	// cudaMalloc((void **) &d_trans_per_thread, bins*sizeof(unsigned long long int));
	// cudaMemcpy(d_trans_per_thread, h_trans_per_thread, bins*sizeof(unsigned long long int), cudaMemcpyHostToDevice);

	//cout << "num input strings " << cur_size_vec.size() << endl;
	...
	*blocksize  = blockSize_init;
	...
	//cout << "grid " << burst.get_sizes().size() << " block " << cfg.get_threads_per_block() << endl;
    ...
    cout << "GPU kernel running ... " << endl;
		...
		nfa_kernel<<<grid, block, max_shmem>>>(/* TODO: the next three are w/o reference! */
												...
												d_match_count, d_match_array, tmp_avg_count,
												d_accum_nfa_table_lengths, d_accum_offset_table_lengths, d_accum_state_vector_lengths,
												d_filter_table, d_filter_symbol_counts, d_filter_symbol_offset, d_helper_table, /* d_trans_per_thread, d_active_bits ,
												d_num_active_groups */
	#ifdef COUNTERS												
												, d_table_access
	#endif												
	...
	// // Copy active groups array from device to host and print data
	// cudaMemcpy(h_active_bits, d_active_bits, BITS*sizeof(unsigned long long int), cudaMemcpyDeviceToHost);
	
	// unsigned long long int total_active = 0;
	// for(int i = 0; i < BITS; i++) {
	// 	total_active += h_active_bits[i];
	// }
	// cout << "total active = " << total_active << endl;
	// cout << "groups processed = " << (total_active - h_active_bits[0]) << endl;

	// for(int i = 0; i < BITS; i++) {
	// 	cout << i << ": " << h_active_bits[i] << endl;
	// 	//cout << i << ": " << h_active_bits[i] << endl;
	// }

	// cout << endl << "Num active frequency" << endl;
	// for(int i = 0; i < BITS; i++) {
	// 	cout << i << ": " << (double)h_active_bits[i]/(double)total_active << endl;
	// 	//cout << i << ": " << h_active_bits[i] << endl;
	// }
	// unsigned long long int active_total = total_active - h_active_bits[0];
	// cout << endl << "Frequency when group active" << endl;
	// for (int i = 1; i < BITS; i++) {
	// 	cout << i << ": " << (double)h_active_bits[i]/(double)active_total << endl;
	// }
	
	// cudaFree(d_active_bits);
	// // //free(h_active_bits);
	
	// // Copy num_active_groups array from device to host and print data
	// cudaMemcpy(h_num_active_groups, d_num_active_groups, active_groups_size*sizeof(unsigned long long int), cudaMemcpyDeviceToHost);

	// // cout << "Number of groups active at a time (bins)" << endl;
	

	// // unsigned long long int active_groups_bins[active_groups_size/50] = {0};
	// // for (int i = 0; i < active_groups_size; i++) {
	// // 	active_groups_bins[i/50] += h_num_active_groups[i];
	// // }

	// // for (int i = 0; i < active_groups_size/50; i++) {
	// // 	cout << "Bin " << i << ": " << active_groups_bins[i] << endl;
	// // }
	// // int total = 0;
	// // cout << endl << "Num active groups" << endl;
	// // for (int i = 0; i < active_groups_size; i++) {
	// // 	cout << i << ": " << h_num_active_groups[i] << endl;
	// // 	total += h_num_active_groups[i];
	// // }
	// // cout << endl << "Total symbols " << total << endl;

	// cudaFree(d_num_active_groups);

	// // Copy transitions per thread array from device to host and print data
	// cudaMemcpy(h_trans_per_thread, d_trans_per_thread, bins*sizeof(unsigned long long int), cudaMemcpyDeviceToHost);
	// // Print all
	// for (int i = 0; i < bins; i++) {
	// 	if (h_trans_per_thread[i] > 0) {
	// 		cout << i << " transitions " << h_trans_per_thread[i] << " times" << endl;
	// 	}
	// }

	// // Group data into bins and print
	// long buckets[7] = {0};

	// for (int i = 0; i < 10; i++) {
	// 	buckets[0] += h_trans_per_thread[i];
	// }
	// for (int i = 10; i < 50; i++) {
	// 	buckets[1] += h_trans_per_thread[i];
	// }
	// for (int i = 50; i < 100; i++) {
	// 	buckets[2] += h_trans_per_thread[i];
	// }
	// for (int i = 100; i < 500; i++) {
	// 	buckets[3] += h_trans_per_thread[i];
	// }
	// for (int i = 500; i < 1000; i++) {
	// 	buckets[4] += h_trans_per_thread[i];
	// }
	// for (int i = 1000; i < 5000; i++) {
	// 	buckets[5] += h_trans_per_thread[i];
	// }
	// for (int i = 5000; i < bins; i++) {
	// 	buckets[6] += h_trans_per_thread[i];
	// }
	

	// cout << endl << "< 10: " << buckets[0] << endl;
	// cout << "< 50: " << buckets[1] << endl;
	// cout << "< 100: " << buckets[2] << endl;
	// cout << "< 500: " << buckets[3] << endl;
	// cout << "< 1000: " << buckets[4] << endl;
	// cout << "< 5000: " << buckets[5] << endl;
	// cout << ">= 5000: " << buckets[6] << endl;

	// cudaFree(d_trans_per_thread);
